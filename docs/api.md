...
| — | Finger smudge (0.1.54) | Not a tool and never started by the engine's own touch handling — the **host** owns the gesture (a finger, never the nib) and drives `beginSmudge()` → `smudgeAlong(points)` per batch → `endSmudge()`. Within `smudgeRadius` (default 32 px) of the sweep the **graphite** pixels are pulled toward the mean of their neighbourhood — `rasterSmudging` sets the pull per pass, the neighbourhood's spread, the feathered edge, the little carried off the tone it converges to (`gamma`, 3: the mean of the neighbourhood's darkness cubed, brought back by the cube root, so a hatch settles as shading rather than a pale wash) and the graphite the finger carries (`carry` px of travel, `deposit` of the load laid under the finger — a rub out of a mark leaves a fading trail) — so a hatch of separate pencil lines runs together into a tone: blended and redistributed, not lifted and not clumped; the graphite pales only by `loss` and by what spreads past the corridor's edge. Once per pixel per pass, again on each reversal of travel — the rubber's pass mask, so a dense digitizer or a slow hand blends no more than a quick one. The ink image is never read, never allocated and never announced. Each batch fires `onRasterWillChange(GRAPHITE, rect)` → blend → `onRasterChanged(GRAPHITE, rect)` exactly like a rub, and `endSmudge()` fires `onPenLifted`, so a host that closes one undo entry per contact needs nothing new. Ratta's direct path writes the blended corridor straight into the panel per batch; the Onyx and generic engines ride the eraser's redraw cadence. **The smear follows the hand (0.1.60):** the neighbourhood is a box turned to the batch's line of travel (its principal axis) — `spread` px along it, `across` px (default 2) to either side — so a rub left and right pushes graphite left and right and a rub along a line only smears it along itself; a batch with no travel uses the plain square box |
| `SMUDGE` | Stylus smudge (0.1.60) | The finger smudge's sweep, driven by the engine's own touch handling from the nib: a stylus contact smudges the **graphite** raster within `smudgeToolRadius` (default 16 px — a stump, half the fingertip) with the same `rasterSmudging`; nothing is drawn or erased, the ink image never read. Each batch announces like a rub and the lift fires `onPenLifted`. On Ratta and Onyx the firmware paints nothing under it (as under `NONE`); the barrel button / eraser end still point-erases. A no-op in stroke mode |
...
batch, the same one entry per contact. **Supernote's answer becomes "not at all" where the
panel is its own (0.1.43):** on a direct raster page the rubbed corridor is written into the
panel as each batch lifts, so a mid-sweep window frame would be a second, later opinion about
pixels the artist can already see, and one redraw at the end of the sweep is the mirror. The
16 ms stands wherever the firmware is still what previews.
...
**On Supernote a raster page previews by painting the panel itself (0.1.41; the whole page
since 0.1.43).** With a raster page open, `gpaper-ratta` opens the panel driver (`/dev/ebc`,
which the vendor's own SELinux policy lets any app open — Supernote's Atelier draws the same
way) and paints straight into it, **with pressure live and nothing to see at pen-up**: what
is on the panel is what the page holds, so the commit changes no pixel. **Every tool on that
page goes through it** — the pencil's graphite fleck by fleck, the pen's ink segment by
segment, and the rubber's corridor as it lifts — and the ink daemon is disabled across the
whole page rather than for one armed tool. **The bake *is* the live layer**: at pen-up the
engine composites what it painted into the page image (plus the pencil's end cap, which a
live preview cannot know until the pen leaves), instead of re-deriving the mark — which is
what makes the mirror exact rather than close, and takes the second that a dense scribble's
...
The constant the one-tone daemon forced on the bake — the pressure 0.5 above — goes back to
...
`penStyle`, eraser via `eraserRadius` (and, on a raster page, `rasterRubbing`; the host-driven finger smudge via `smudgeRadius` + `rasterSmudging`, 0.1.54; the stylus `Tool.SMUDGE` via `smudgeToolRadius`, 0.1.60). Finger input is never a tool — it passes through
