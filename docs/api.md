...
`penStyle`, eraser via `eraserRadius` (and, on a raster page, `rasterRubbing`; the host-driven finger smudge via `smudgeRadius` + `rasterSmudging`, 0.1.54). Finger input is never a tool — it passes through
