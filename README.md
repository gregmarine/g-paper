...
- **Pen-gesture recognizers (opt-in)** — smart lasso (a quick closed pen loop around content
  selects it) and scribble erase (a dense zigzag crosses out the strokes it touches and the
  host content objects it travels through); both default off, pen-tool only, and fall through
  to ordinary ink when they catch nothing.
...
Publish to mavenLocal (0.1.48 is mavenLocal-only), add `mavenLocal()` to your repositories, then:

```kotlin
dependencies {
    implementation("com.symmetricalpalmtree.gpaper:gpaper-core:0.1.48")
    implementation("com.symmetricalpalmtree.gpaper:gpaper-onyx:0.1.48")  // only if you target BOOX
    implementation("com.symmetricalpalmtree.gpaper:gpaper-ratta:0.1.48") // only if you target Supernote
}
```
