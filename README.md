...
Publish to mavenLocal (0.1.41 is mavenLocal-only), add `mavenLocal()` to your repositories, then:

```kotlin
dependencies {
    implementation("com.symmetricalpalmtree.gpaper:gpaper-core:0.1.41")
    implementation("com.symmetricalpalmtree.gpaper:gpaper-onyx:0.1.41")  // only if you target BOOX
    implementation("com.symmetricalpalmtree.gpaper:gpaper-ratta:0.1.41") // only if you target Supernote
}
```
